...
const packMessageToGroup = (newMessageList) => {
    const groups = [];
...
export const combineMessageGroup = (groupBefore, groupAfter) => {
    let messageGroupsToSave = [];
    const lastGroupInBefore = groupBefore[groupBefore.length - 1];
    const firstGroupInAfter = groupAfter[0];
    if (!(firstGroupInAfter === null || firstGroupInAfter === void 0 ? void 0 : firstGroupInAfter.query)) {
...
export const createConversationStore = () => create()((set, get) => {
    const clearConversation = () => {
        set(Object.assign({}, defaultData));
        get().closeSendMessage();
    };
    return Object.assign(Object.assign({}, defaultData), { setConversationDetail: (detail, messageList) => {
            clearConversation();

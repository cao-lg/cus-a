import { request } from "@tarojs/taro";
import { EventSourceBase, MessageEvent, EventJsonDataType, } from "./event-source";
import { logger } from "../logger";
export class WxEventSource extends EventSourceBase {
    constructor() {
        super(...arguments);
        this.chunkedData = "";
        this.dataType = "unknown";
    }
    _sendMessage() {
        this.task = request({
            url: this.url,
            method: this.method,
            header: this.header,
            data: this.data,
            enableChunked: true,
            enableCookie: true,
            fail: (err) => {
                this.event.trigger(MessageEvent.ERROR, err);
            },
            complete: () => {
                this.close();
            },
        });
        const destroyHeaderReceivedHandler = this.onHeadersReceived();
        const destroyChunkReceivedHandler = this.onChunkReceived();
        this.offTaskHandler = () => {
            destroyHeaderReceivedHandler();
            destroyChunkReceivedHandler();
        };
    }
    close() {
        var _a, _b;
        if (!super.close()) {
            return false;
        }
        (_a = this.task) === null || _a === void 0 ? void 0 : _a.abort();
        (_b = this.offTaskHandler) === null || _b === void 0 ? void 0 : _b.call(this);
        return true;
    }
    getDataType(header) {
        const [, contentTypeRaw] = Object.entries(header).find(([key]) => {
            if (key.toLowerCase() === "content-type") {
                return true;
            }
            return false;
        }) || [];
        const contentType = contentTypeRaw || "";
        if (contentType === null || contentType === void 0 ? void 0 : contentType.includes("text/event-stream")) {
            return "stream";
        }
        else if (contentType === null || contentType === void 0 ? void 0 : contentType.includes("application/json")) {
            return "json";
        }
        return "unknown";
    }
    onHeadersReceived() {
        var _a;
        const func = (res) => {
            this.dataType = this.getDataType(res.header);
            this.event.trigger(MessageEvent.OPEN, res.header);
            logger.debug("onHeadersReceived", res.header);
        };
        (_a = this.task) === null || _a === void 0 ? void 0 : _a.onHeadersReceived(func);
        return () => {
            var _a;
            (_a = this.task) === null || _a === void 0 ? void 0 : _a.offHeadersReceived(func);
        };
    }
    onChunkReceived() {
        var _a;
        const func = (res) => {
            const uint8Array = new Uint8Array(res.data);
            const textDecoder = new TextDecoder("utf-8");
            const data = textDecoder.decode(uint8Array);
            logger.debug("onChunkReceived raw data", data);
            logger.debug("onChunkReceived data type", this.dataType);
            if (this.dataType === "json") {
                this.event.trigger(MessageEvent.MESSAGE, {
                    event: EventJsonDataType,
                    data: data,
                });
                return;
            }
            // 处理返回的json字符串
            this.chunkedData += data;
            if (data.slice(-2) === "\n\n") {
                const messageEvents = this.unpackData(this.chunkedData);
                messageEvents.map((message) => {
                    this.event.trigger(MessageEvent.MESSAGE, message);
                });
                this.chunkedData = "";
                logger.debug("onChunkReceived messageEvents", messageEvents);
            }
            else {
                logger.debug("onChunkReceived messageEvents is not complete");
            }
        };
        (_a = this.task) === null || _a === void 0 ? void 0 : _a.onChunkReceived(func);
        return () => {
            var _a;
            (_a = this.task) === null || _a === void 0 ? void 0 : _a.offChunkReceived(func);
        };
    }
    unpackData(data) {
        const lines = data.split("\n");
        const messageEvents = [];
        let i = 0;
        while (i < lines.length - 1) {
            if (lines[i].startsWith("event:") && lines[i + 1].startsWith("data:")) {
                const eventName = lines[i].slice(6).replace(/\s/g, "");
                const eventData = lines[i + 1].slice(5).replace(/\s/g, "");
                messageEvents.push({
                    event: eventName,
                    data: eventData,
                });
                i += 2;
            }
            i++;
        }
        return messageEvents;
    }
}
//# sourceMappingURL=wx-event-source.js.map
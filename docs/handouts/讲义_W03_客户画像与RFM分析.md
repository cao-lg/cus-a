...
<!-- -->
```
-   **画像数据：**年龄、性别、地区、职业、兴趣偏好

这三个维度组合在一起，就构成了所谓的"360度客户视图"。
...
<!-- -->
```
-   **淘宝：**"千人千面"推荐背后，是数亿用户的精细化画像------你的每一次点击、每一次停留、每一次购买，都在为你的画像添砖加瓦
...
<!-- -->
```
-   标准差越大，说明客户之间的差异越大------这时候就需要考虑客户分群，而不是用一套策略对待所有客户

**2.3 RFM模型：客户价值的三把尺子（核心重点）**
...
<!-- -->
```
-   用Kimi辅助生成分析："请分析这个数据集中客户的年龄分布和消费金额分布"
...
<!-- -->
```
-   将R、F、M分别按五分制打分（R越小分越高，F和M越大分越高）
...
<!-- -->
```
-   M评分：M≥5000=5分，≥3000=4分，≥1000=3分，≥500=2分，\<500=1分

**提示：**客户A的R=5天≤7天，所以R得5分；F=20次≥15次，所以F得5分；M=8500≥5000，所以M得5分。RFM总分=15分，属于高价值客户。

**练习2：客户分群策略（进阶）**

针对计算出的RFM评分，回答以下问题：

1.  客户A属于什么类型？应该采取什么策略？

2.  客户B属于什么类型？如何挝回？

3.  如果你是「潮玩星球」的运营经理，你会优先投入资源维护哪类客户？为什么？
